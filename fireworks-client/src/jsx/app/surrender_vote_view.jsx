define(['jquery', 'React', 'app/log'], function ($, React, Log) {
    ...
    var SurrenderVoteView = React.createClass({
        getInitialState() {
            return {
                px: 0,
                py: 0,
        ...
        componentDidMount() {
            ...
            TweenLite.to(".progress-bar", 59, {width: 0, ease: Power0.easeNone, onComplete: () => {
                this.props.onClose();
            }});
        },
        componentDidUpdate() {
        ...
        hideOptions() {
            ...
            TweenLite.to(".your-choice", 0.3, {autoalpha: 1, display:'block', height: '1vh', onComplete: () => {
        ...
        render() {
            ...
            voteViews = $.map(votes, (val) => {
                return (<div className={val == 1 ? "vote-yes" : "vote-no"}></div>);
            });

            voteViews.push(
                <div style={{flex: (numPlayers - votes.length) + " 0 0"}}>
                </div>
            );

            return (
                <div className="surrender-vote-view" style={{left: px, top: py}} 
                    onMouseUp={this.onMouseUp} 
                    onMouseMove={this.onMouseMove} 
                    onMouseDown={this.onMouseDown}>

                    <p>Surrender</p>
                    <div className="votes-container">{voteViews}</div>
                    <div className="progress-bar"></div>
                    <div className={enableVote ? "options-container" : "gone"}>
                        <button className="theme-button" onClick={this.onNoClick}>No</button>
                        <button className="theme-button" onClick={this.onYesClick}>Yes</button>
                    </div>
                    <p className={enableVote ? "choice-hidden your-choice" : "your-choice"}>{this.state.choice}</p>
                </div>
            );
        }
    });
